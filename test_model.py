...
class Model:
    ...
    def add_residual_block_strict(self, num_units, mapsize = 3, stddev_factor = 2.0):
        assert len(self.get_output().get_shape()) == 4 and "Previous layer must be 4-dimensional (batch, width, height, channels)"
        if num_units != int(self.get_output().get_shape()[3]):
            self.add_conv2d(num_units, mapsize=1, stride=1, stddev_factor=1.)
...
def _generator_model_with_scale(sess, features, labels, masks, channels=2, layer_output_skip=5,
    ...
    if num_dc_layers >= 0:
        ...
        for index_dc_layer in range(num_dc_layerss):
            ...
            corrected_real = tf.reshape(tf.real(corrected_complex), [FLAGS.batch_size, 28, 28, 1])
            corrected_imag = tf.reshape(tf.imag(corrected_complex), [FLAGS.batch_size, 28, 28, 1])
           
            #print('size_corrected_real', corrected_real.get_shape())

            corrected_real_concat = tf.concat([corrected_real, corrected_imag], axis=3)
            #corrected_real_abs = tf.abs(corrected_complex)
...
def create_model(sess, features, labels, masks, architecture='var'):
    ...
    with tf.variable_scope('disc') as scope:
        ...
        disc_real_output, disc_var_list, disc_layers = \
                _discriminator_model(sess, disc_real_input, hybrid_disc=FLAGS.hybrid_disc)

        scope.reuse_variables()
        disc_fake_output, _, _ = _discriminator_model(sess, gene_output, hybrid_disc=FLAGS.hybrid_disc)

    
        #test
        #scope.reuse_variables()
        #disc_moutput, _, disc_mlayers = \
        #        _discriminator_model(sess, gene_moutput, hybrid_disc=FLAGS.hybrid_disc)
